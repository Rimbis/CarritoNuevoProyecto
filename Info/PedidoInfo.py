...
class PedidoInfo:
    def obtener_Pedidos(self, cursor: psycopg.Cursor) -> list:
        res = cursor.execute(
            "SELECT Producto.Tipo, producto.Precio, Cliente.Nombre, Cliente.Apellido FROM pedido INNER JOIN Cliente ON pedido.id_cliente = Cliente.id_cliente INNER JOIN producto ON pedido.id_producto = producto.id_producto"
        ).fetchall()
        return [
